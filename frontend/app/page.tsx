...
export default function HomePage() {
  ...
  // Show carousel arrows whenever there's more than one course
  const hasCourseCarousel = ongoingCourses.length > 1;

  const scrollCourses = (direction: 'left' | 'right') => {
    if (!coursesScrollRef.current) return;
    const cardWidthWithGap = 404; // approximate card width + gap
    const current = coursesScrollRef.current.scrollLeft;
    const next =
  ...
  // Auto-scroll ongoing courses when carousel is active
  useEffect(() => {
    if (!hasCourseCarousel || !ongoingCourses.length) return;
  ...
  }, [hasCourseCarousel, ongoingCourses.length]);
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Ongoing / Featured Courses - horizontal cards like design */}
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {/* Navigation arrows - only when more than 3 courses */}
            {hasCourseCarousel && (
              <>
                <button
            ...
            <div
              ...
              className={`flex gap-6 overflow-x-auto hide-scrollbar ${
                hasCourseCarousel ? 'scroll-smooth' : 'justify-center'
              }`}
            >
              {ongoingCourses.length > 0 ? (
                ongoingCourses.map((course) => (
                  <div key={course.id} className="flex-shrink-0 w-[400px]">
                    <Link href={`${ROUTES.COURSES}/${course.id}`}>
                      <div className="bg-white border border-gray-200 shadow-lg overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-200">
                        {course.thumbnail && (
                      ...
                      <p className="mt-3 text-base md:text-lg font-semibold text-center tracking-wide text-[var(--foreground)]">
                        {course.title}
                      </p>
                    </Link>
                  </div>
                ...
                <div className="w-full text-center text-[var(--muted-foreground)]">
